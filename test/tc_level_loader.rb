...
class TC_LevelLoader < Test::Unit::TestCase
	...
	def test_instance
		ins = LevelLoader.new "original.xsb"
		assert_equal true, ins.instance_of?(LevelLoader)
	end
	
	def test_NoFileError
		assert_raise(NoFileError) do
			ins = LevelLoader.new "inexistant"
		end
	end
	
	def test_title_of_set
		ins = LevelLoader.new "test_file1.xsb"
	...
	def test_level_1
		ins = LevelLoader.new "test_file2.xsb"
